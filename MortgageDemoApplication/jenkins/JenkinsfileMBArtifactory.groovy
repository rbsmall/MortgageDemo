...
def zAppBuild       = '~/dbb-zappbuild/build.groovy'
def zAppPkg         = '~/Pipeline-scripts/utilities/package.groovy'
def pdsHLQ          = 'BRICE.JENKINS.PIPELINE'
def jobRepo         = 'git@github.ibm.com:brice/MortgageV2.git'
def gitCredId       = 'briceGithub'
def artifactoryURL  = 'https:/eu.artifactory.swg-devops.com/artifactory'
def artifactoryRepo = 'sys-dat-team-generic-local'
...
pipeline {
	...
	environment {
		...
		BUILDID         = Calendar.getInstance().getTime().format('YYYY-MM-dd_hh:mm:ss',TimeZone.getTimeZone('EDT'))
		apiKeyLoc       = '/u/bsmall/Archive/myApiKey.txt'
	...
	stages {
		...
		stage('Impact Build') {
			...
			steps {
				  println  '** Building..'
				  sh "pwd"
				  sh "${env.DBB_HOME}/bin/groovyz -Dlog4j.configurationFile=file:/var/dbb109/conf/log4j2.properties ${zAppBuild} -w ${env.wkDir} -a ${myApp} -o ${WORKSPACE}/buildlogs/build-${BUILD_NUMBER} -h ${pdsHLQ} -id ADMIN -pw ADMIN --impactBuild"
			}
		}
		...
		stage('Packaging Artifacts') {
			...
			steps {
				  ...
				  sh "pwd"
				  script{
					  artifacts = sh(returnStdout: true, script: "ls -d buildlogs/build*")
//				      artifacts = sh(returnStdout: true, script: "ls -d \"${env.wkDir}\"/buildlogs/build*")  // how to use variables.  Must escape the quotes
				  }

				  println "Artifacts - ${artifacts}"
				  sh "groovyz -Dlog4j.configurationFile=file:/var/dbb109/conf/log4j2.properties ${zAppPkg} -workDir ${env.wkDir} -application ${myApp} -version ${env.BUILDID} -s ${jobRepo} -g ${jobRepo} -x ${env.BRANCH_NAME} -y ${env.BRANCH_NAME} -b 12345678 -n P092259 -u arty.none.com-TBD -workSpace ${env.WORKSPACE}/${artifacts}"
			}
		}
		...
		stage('Publishing Artifacts') {
			...
			steps {
				  ...
				  script {
				  	  myApiKey = sh(returnStdout: true, script: "cat /u/bsmall/Archive/myApiKey.txt")
				  }
				  println "** API Key - ${myApiKey}"
				  sh "curl --silent --insecure -H X-JFrog-Art-Api:${myApiKey} -T ${env.wkDir}/tempLoadDir/${myApp}.tar ${artifactoryURL}/${artifactoryRepo}/brice/${myApp}/${env.BUILDID}.tar"
			}
		}
	}
